...
private:
    ...
    struct StageRobot
    {
        ...
        std::vector<ros::Publisher> fiducial_pubs; // multiple fiducial detectors
    ...
    for (size_t r = 0; r < this->positionmodels.size(); r++)
    {
        ...
        for (size_t s = 0; s < this->fiducialmodels.size(); s++)
        {
            if (this->fiducialmodels[s] and this->fiducialmodels[s]->Parent() == new_robot->positionmodel)
            {
                ...
                ROS_INFO( "subscribed to Fidutial model \"%s\"", this->fiducialmodels[s]->Token() ); 
            }
        }
	...
	// TODO - print the topic names nicely as well
        ...
        for (size_t s = 0;  s < new_robot->fiducialmodels.size(); ++s)
        {
            if (new_robot->fiducialmodels.size() == 1)
                new_robot->fiducial_pubs.push_back(n_.advertise<markers_msgs::Markers>(mapName(MARKERS, r, static_cast<Stg::Model*>(new_robot->positionmodel)), 10));
            else
